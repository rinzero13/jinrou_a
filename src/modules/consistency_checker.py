...
LLM_MODEL = "gpt-4o-mini" # Agent.pyと同じモデルを使用
...
class LogicalConsistencyChecker:
    ...
    def _create_checker_prompt(self, game_info: Info, talk_history: list[Talk], virtual_talk: str, agent_name: str) -> tuple[str, str]:
        ...
        system_message = (
            ...
            "   - **2〜3ターン先の議論展開**を見据えた、**深い論理的根拠**を伴う主張か？理由を述べずに同意する場合、論理的一貫性なしと判定する。\n" # 【同調・短絡的対話の対策】
            
            "**【戦略の柔軟性に関する判定基準】**:\n"
            "発話が**自身の真の役職や勝利目標を間接的に示唆する内容**（例：人狼のCO、特定の陣営の勝利を直接的に目指す発言）であっても、それが**ゲーム状況全体から見て、戦略的に一貫性のある論理的根拠**に基づき、**論理的に成立しうる高リスク戦略**であるならば、論理的一貫性ありと判定してください。ただし、その戦略的根拠が発話内容から論理的に推測できる必要があります。\n" # 【戦略の柔軟性確保】
