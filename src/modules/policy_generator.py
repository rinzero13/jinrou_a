...
class UtterancePolicyGenerator:
    ...
    def get_planning_prompt_instructions(self, role: Role) -> str:
        """M4: 主張の核の決定と応答方針をJSONで決定させるための指示を生成する。"""
        instructions = ""
        instructions += (
            "【M4: 初期計画と議論の焦点決定】\n"
        ...
        instructions += (
            ...
            "あなたのcore_goalは、以下の7つの分類のいずれかに該当するように決定してください。\n"
            "この分類から最も戦略的に有利なものを選択し、`classification_type`として出力してください。\n"
            "| 分類タイプ | 目的（Goal） | 具体例（達成したい状態） |\n"
            "| :--- | :--- | :--- |\n"
            "| SelfEstablish | 自己の正当性の確立/情報公開 | 自身の役職COや、真/偽の証拠提示により信頼を得る。 |\n"
            "| Attack | 他者の不審点/矛盾の指摘 | 特定プレイヤーへの告発を行い、議論の焦点を集める。 |\n"
            "| Guide | 処刑ターゲットの誘導/決定 | 特定の投票推奨を行い、村の合意形成を加速させる。 |\n"
            "| Inquire | 特定の情報収集/場への問いかけ | 未確定情報（他者の役職や信念）を知るための質問を投げる。 |\n"
            "| Support | 味方の擁護/陣営の支援 | 告発された味方を弁護し、議論を自身に有利な方向へ戻す。 |\n"
            "| Disrupt | 情報撹乱・場全体の混乱 | 議論の焦点を意図的にずらし、村の判断力を低下させる。 |\n"
            "| Passive | 議論の観察/情報不足時の沈黙 | 重要な情報や発言が出るまで待ち、不要な発言を避ける。 |\n"
        )
        ...
        instructions += (
            ...
            "2. 直近3ターンの自分の発言や、議論が停滞している論点の繰り返しを避け、議論を次の段階に進める新しい視点や反証を優先的に提示する目標を設定してください。\n"
            "【論理的深度の要請】：\n"
        ...
        self.logger.logger.debug("M4 Planning instructions updated with argument classification.")
        return instructions