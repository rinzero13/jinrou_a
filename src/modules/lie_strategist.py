...
class LieStrategyModule:
    ...
    def get_strategy_decision_instructions(self, role: Role) -> str:
        """
        M5: M4の計画に基づき、嘘戦略の有無を決定し、リスク評価と目標の拡張をJSONで決定させるための指示を生成する。
        """
        ...
        instructions = (
            ...
            "【戦略的柔軟性の確保】：\n"
            "人狼陣営（人狼・狂人）である場合、パワープレイ（自身の役職のCOなど）といった高リスク戦略も、陣営の最終的な勝利に繋がる明確な論理的裏付けがある場合に限り採用を検討できます。単なる短期的な混乱を目的とするのではなく、多ターンにわたる戦略的利益を評価してください。\n"
        )
        ...
        instructions += (
            ...
            "| :--- | :--- | :--- |\n"
            "| FactualDeception | 事実の偽造・虚偽: 役職、占い結果、襲撃ターゲットなど、ゲーム内の核となる事実について嘘をつく。 | 高リスク・高リターン（真偽が明確になりやすいため） |\n"
            "| ConsistencyBreak | 一貫性の偽装: 過去の行動や発言と矛盾しないように装う、または意図的に矛盾を突いて混乱を誘う。 | 中リスク・中リターン（ログ検証に依存するため） |\n"
            "| Omission/Distortion | 情報隠蔽・歪曲: 事実の一部を語らない、または意図的に解釈を誇張/矮小化して誘導する。 | 低リスク・低〜中リターン（嘘がバレにくいが、効果も限定的） |\n"
            "| DirectBluff | 直接的なブラフ: 証拠なしで、次の行動（投票/襲撃）について誤った宣言をする。 | 中リスク・中リターン（即時的な行動誘導に強い） |\n"
        )
        ...
        self.logger.logger.debug("M5 Lie strategy decision instructions generated with explicit 'false' pattern.")
        return instructions